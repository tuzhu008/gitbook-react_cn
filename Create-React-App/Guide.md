...
编辑器显示Lint输出
> **[info]** 注意: 在 `react-scripts@0.2.0` 和更高版本上，这个特性是可用的 <br>
> 它只能在 npm 3 或更高版本上工作.
...
```
> **[info]** 注意: 如果通过[HOST 或 PORT 环境变量][121] 进行调整，URL可能会有所不同
...
> **[info]** 注意: 如果通过[HOST 或 PORT 环境变量][124] 进行调整，URL可能会有所不同 .
...
> **[info]** 注意: 使用不同的预处理器, 依照你的预处理器文档替换 `build-css` 和 `watch-css` 命令。
...
使用`public`文件夹
> **[info]** 注意: 这个特性要在 `react-scripts@0.5.0` 和更高的版本才可用
...
> **[info]** 注意: 这个特性需要在 `react-scripts@0.2.3` 和 更高的版本.
...
> **[info]** 注意: 你必须从`REACT_APP_`开始创建自定义环境变量。除 `NODE_ENV`之外的任何其他变量都将被忽略[在机器上公开一个可能具有相同名称的私有密匙][167]. 更改任何环境变量将要求您在运行时重新启动开发服务器。
...
> **[info]** 注意: 这个特性需要在 `react-scripts@0.9.0` 和更高版本上使用.
...
在`.env`中添加开发环境变量
> **[info]** 注意: 这个特性需要 `react-scripts@0.5.0` 和更高版本.
...
其他一些可以使用`.env`文件吗？
> **[info]** 注意: 这个特性 **在 `react-scripts@1.0.0` 和更高版本中可以使用**.
...
> **[info]** 注意: 如果您正在为开发定义环境变量，那么您的CI 和/或 托管平台最有可能需要这些定义。参考他们的文档如何做到这一点。例如，请参阅[Travis CI][170] 或者 [Heroku][171]。
...
开发模式下代理API请求
> **[info]** 注意: 这个特性需要 `react-scripts@0.2.3` 和更高版本.
...
手动配置代理
> **[info]** 注意: 这个特性需要 `react-scripts@1.0.0` 和更高的版本.
...
在开发中使用HTTPS
> **[info]** 注意: 这个特性需要 `react-scripts@0.4.0` 和更高的版本.
...
> **[info]** 注意: 这个特性需要 `react-scripts@0.3.0` 和更高版本.<br>
...
初始化测试环境
> **[info]** 注意: 这个特性需要 `react-scripts@0.4.0` 和更高版本.
...
> **[info]** 注意: 这个特性需要 `react-scripts@0.9.0` 和更高版本.
...
### [GitHub Pages][286]
GitHub页面
> **[info]** 注意: 这个特性需要 `react-scripts@0.2.0` 和更高.
